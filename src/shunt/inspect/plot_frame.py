...
# One contract for every figure this repo draws — the benchmark halves (routing +
# escalation, which reach it through `benchmark.plot_frame`) and the shipped
# diagnostics. It lives in the wheel because the shipped figure code needs it and
# SH006 forbids src/shunt importing benchmark.
#
...
# MANDATORY in code. They are written to the manifest named by `Provenance.manifest`,
# and SH009 holds that manifest in bijection with the per-figure sections of the
# matching docs page. The explanation did not get deleted; it moved to where prose
# belongs, and a gate now proves it is there.
#
...
# Agg before pyplot loads, so tests and the container render headless without a display.
...
def subplots(
    ...
    nrows: int = 1,
    ncols: int = 1,
    **kwargs: Any,  # noqa: ANN401 (a pass-through to `Figure.subplots`)
) -> tuple[Figure, Any]:
...
def save(
    ...
    *,
    ...
    # SH007 denies savefig anywhere else, so every figure is framed and
