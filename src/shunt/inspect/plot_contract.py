...
#
# This runs INSIDE shunt.inspect.plot_frame.save() under SHUNT_PLOT_STRICT=1, which the test
...
def audit(fig: Figure, *, band_top_px: float | None = None) -> list[Violation]:
    ...
    # `get_renderer` is untyped on the Agg stubs; the cast above already pins the class.
    renderer: RendererBase = cast("FigureCanvasAgg", fig.canvas).get_renderer()  # type: ignore[no-untyped-call]
    canvas = fig.bbox
