...
class TestTheVerdictIsAttachedToWhatItCertifies:
    ...
    def test_the_figure_footer_states_the_verdict(
        self, monkeypatch: pytest.MonkeyPatch, tmp_path: Path
    ) -> None:
        # Carrying the verdict on the object is only half of it — a reader looking at the PNG has
        # to see it too, so the first footer note is the verdict itself.
        from benchmark.admissibility import admissibility_verdict
        from benchmark.routing.scripts import plot_knn_nulls

        captured: dict[str, object] = {}

        def fake_save(fig, out_path, spec, extra=None):  # noqa: ANN001, ANN202
            captured["notes"] = () if extra is None else extra.notes
            return out_path

        monkeypatch.setattr(plot_knn_nulls.plot_frame, "save", fake_save)
        rejected = admissibility_verdict(0.51, 0.50, chance_level=0.5, chance_band=0.1)
        curve = knn_nulls.TransferCurve(
            ...
            null_mean=(0.5,),
            null_sd=(0.05,),
            max_null=knn_nulls.Band(mean=0.5, sd=0.05, lo=0.4, hi=0.6, n=200),
            best_constant=0.5,
            best_constant_model="m",
            n_tasks=10,
            n_perm=200,
        )
        plot_knn_nulls.plot_transfer_curve(curve, tmp_path / "curve.png")
        notes = captured["notes"]
        assert isinstance(notes, tuple)
        assert notes[0] == rejected.reason
        assert any(str(n).startswith("NOT QUOTABLE") for n in notes)
