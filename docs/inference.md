...
Each figure below is the PNG committed under `docs/assets/figures/inference/`. The
image carries its claim, its sample size and — where a reader could be actively
misled — one red line. The rest is here, and it is generated from the same manifest
the renderer writes, not retyped.
...
- **This page measures a corpus, not a deployment.** The committed render has no live
  stratum, so every live claim on it is empty by construction. Nothing here is
  evidence that the router behaves well — or badly — in production; it is evidence
  that the instrument is honest about what it can and cannot see.
- **Session-cadence, not request-cadence.** Every count is a session. A session that
  served many turns appears once, so none of these figures is a request-rate view.
- **Cost is the provider's reported figure.** A provider that under-reports cache
  reads under-reports here too, and the unknown-cost bar is the only signal of it.
- **Escalation holds are a lower bound.** A directive that said raise, on a boundary
  where no rung could be delivered, returns early with the served model unchanged and
  writes no hold-reason token at all. F6 recovers that case from the voided
  exploration record where escalation happened to be exploring, and counts it as a
  derived bar — where it was not exploring, the hold leaves no record anywhere.
- **An ADMISSIBLE instrument verdict is a breakage gate, not an accuracy warrant.**
  It says the pipeline still recovers a planted signal and still collapses on a
  destroyed one. It does not say the off-policy numbers are accurate to within a few
  points.
- **The neighbourhood figure reads the embedding space, not time.** Leave-one-out over
  a corpus imported in a single burst measures the corpus; it says nothing about how
  the router's neighbourhoods evolve as live rows accumulate.

Related: [routing](routing.md) and [escalation](escalation.md) measure the same two
decisions offline against the benchmark corpus; [results](results.md) is where the
kill-gate verdict lives.
